(ns bizassoc.operation
  "OperationActor -- one association operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (AssocOps-
  LLM) is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Association Governance Governor (:govern) and the
  ...
  One graph run = one association operation (intake -> advise -> govern
  ...
  governor is clean -- see `bizassoc.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [bizassoc.assocopsllm :as assocopsllm]
            [bizassoc.governor :as governor]
...
(defn build
  ...
  `bizassoc.store/Store`).
  opts:
    :advisor      -- a `bizassoc.assocopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (assocopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AssocOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (assocopsllm/-advise advisor store request)]
            {:proposal p :audit [(assocopsllm/trace request p)]})))

      ;; Association Governance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
